...
extern "C" {
    __declspec(dllexport) void AdjustBrightness(unsigned char* pixel_data, int img_height, int img_width, int brightness_adjustment)
    {
        ...
        //cv add or cv subtract will handle clamping values such that they cannot go below 0 or 255 for 8 bit values
     ...
     /*   if (brightness_adjustment > 0)
        ...
        {
            cv::subtract(img, cv::Scalar(brightness_adjustment), result);
        }*/
